...
import { DailyInsight, RawFeedItem, Category, WeeklyReview, NewsItem } from "../types";
...
export const processDailyFeeds = async (rawItems: RawFeedItem[]): Promise<DailyInsight[]> => {
...
export const generateWeeklyReview = async (bookmarks: NewsItem[]): Promise<WeeklyReview> => {
  if (!apiKey) throw new Error("API Key is missing");
